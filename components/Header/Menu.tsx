...
export default function (props) {
  ...
  const [underlineParams, editUnderlineParams] = useState({
    width: "0px",
    marginLeft: "-2px",
  });
  ...
  useEffect(() => {
    let elem = document.getElementById(
      "menuItemId" + tabManager.current.toString()
    );
    ...
    window.onresize = () => {
      ...
      editUnderlineParams({
        width: elem.offsetWidth.toString() + "px",
        marginLeft:
          (elem.offsetLeft - elemMenu.offsetLeft - 2).toString() + "px",
      });
    };
  }, [tabManager.current]);
  useEffect(() => {
    editUnderlineParams({
      width:
        document
          .getElementById("menuItemId" + tabManager.current.toString())
    ...
    window.addEventListener("scroll", () => {
      ...
      } else elem.setAttribute("onScroll", "false");

      let lastActiveI = tabManager.current;
      for (var i = 0; i < tabs.length; i++) {
        if (window.scrollY >= elems[i]?.offsetTop - 300) lastActiveI = i;
      }
  ...
  return (
    ...
    >
      ...
      >
        {tabs.map((e, i) => {
          return (
            // @ts-ignore
            <div
              title={"Кнопка перехода к разделу"}
              active={(i === tabManager.current).toString()}
              id={"menuItemId" + i.toString()}
              onMouseEnter={() => {
      ...
      <div
        ...
        style={{
          width: underlineParams["width"],
          marginLeft: underlineParams["marginLeft"],
        }}
      />

      <div
        className={styles.touchCloseHiddenPanel}
        style={{
          height: "100vh",
          width: "100vw",
          position: "absolute",
        ...
        onClick={() => {
          if (props.open) props.setOpenMenu(false);
        }}
      />
    </div>
  );
